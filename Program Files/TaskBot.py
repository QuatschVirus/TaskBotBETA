...
def TaskGUI(task):
    ...
    Date = task['Date']
    DueDateString = str(DueDate[0]) + '.' + str(DueDate[1]) + '.' + str(DueDate[2]) + ' ' + str(DueDate[3]) + ':' + str(DueDate[4])
    DateString = str(Date[0]) + '.' + str(Date[1]) + '.' + str(Date[2]) + ' ' + str(Date[3]) + ':' + str(Date[4])
    if task['Upload'] == True:
        UploadString = 'Ja'
    else:
        UploadString = 'Nein'
...
def addTask(name, description, dueDate, priority, upload):
    Date = [time.localtime().tm_mday, time.localtime().tm_mon, time.localtime().tm_year, time.localtime().tm_hour, time.localtime().tm_min]
    Done = False
...
def NewTask():
    ...
    def Format():
        addTask(Task.get(), Description.get(), [int(Day.get()), int(Month.get()), int(Year.get()), int(Hour.get()), int(Minute.get())], Priority.get(), bool(Upload.get()))
        newTask.destroy()
...
def main():
    ...
    Label(tk, text='===============================================').pack()
    Button(tk, text='Aufgaben verwalten', command=ManageTasks).pack()
    Label(tk, text='===============================================').pack()
    Button(tk, text='Benachrichtigungen prüfen', command=checkPing).pack()
    Label(tk, text='===============================================').pack()
    Button(tk, text='Schließen', command=exit).pack()
    Label(tk, text='===============================================').pack()
    Label(tk, text='Version 1.3b | TaskBot | 18.03.2021 | 11:30').pack()
    checkPing()
    mainloop()
